...
module Unibee
  class UnibeeApiCheckoutSubscriptionCreatePreviewRes
    ...
    def self.openapi_types
      {
        ...
        :'quantity' => :'Integer',
        :'sign_in' => :'UnibeeApiBeanCheckoutSignIn',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiCheckoutSubscriptionCreatePreviewRes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiCheckoutSubscriptionCreatePreviewRes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sign_in')
        self.sign_in = attributes[:'sign_in']
      end
    ...
    def hash
      [addon_params, addons, apply_promo_credit, currency, discount, discount_amount, discount_message, email, gateway, invoice, origin_amount, other_active_subscription_id, other_pending_crypto_subscription, plan, quantity, sign_in, subscription_amount_excluding_tax, tax_amount, tax_percentage, total_amount, trial_end, user_id, vat_country_code, vat_country_name, vat_number, vat_number_validate, vat_number_validate_message].hash
    end
