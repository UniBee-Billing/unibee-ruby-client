...
module Unibee
  class MerchantSubscriptionNewOnetimeAddonPreviewPost200ResponseData
    attr_accessor :addon
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::MerchantSubscriptionNewOnetimeAddonPreviewPost200ResponseData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::MerchantSubscriptionNewOnetimeAddonPreviewPost200ResponseData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [addon, apply_promo_credit, currency, discount, discount_amount, discount_message, email, invoice, origin_amount, quantity, tax_amount, tax_percentage, total_amount, user_id, vat_number].hash
    end
