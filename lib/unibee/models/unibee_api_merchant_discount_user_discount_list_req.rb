...
module Unibee
  ...
  class UnibeeApiMerchantDiscountUserDiscountListReq
    # The discount's Id
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiMerchantDiscountUserDiscountListReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiMerchantDiscountUserDiscountListReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'child_code')
        self.child_code = attributes[:'child_code']
      end
    ...
    def hash
      [id, child_code, count, create_time_end, create_time_start, email, page, plan_ids, sort_field, sort_type, status, subscription_ids, user_ids].hash
    end
