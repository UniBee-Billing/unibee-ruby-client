...
module Unibee
  class UnibeeApiBeanMerchantMetric
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiBeanMerchantMetric` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiBeanMerchantMetric`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'metric_description')
        self.metric_description = attributes[:'metric_description']
      end
    ...
    def hash
      [aggregation_property, aggregation_type, archived, code, create_time, gmt_modify, id, merchant_id, meta_data, metric_description, metric_name, type, unit].hash
    end
