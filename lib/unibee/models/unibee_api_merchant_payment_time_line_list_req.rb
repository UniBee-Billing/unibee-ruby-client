...
module Unibee
  class UnibeeApiMerchantPaymentTimeLineListReq
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiMerchantPaymentTimeLineListReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiMerchantPaymentTimeLineListReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'timeline_types')
        if (value = attributes[:'timeline_types']).is_a?(Array)
          self.timeline_types = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          status == o.status &&
          timeline_types == o.timeline_types &&
          user_id == o.user_id
    end
    ...
    def hash
      [amount_end, amount_start, count, create_time_end, create_time_start, currency, gateway_ids, page, search_key, sort_field, sort_type, status, timeline_types, user_id].hash
    end
