...
module Unibee
  ...
  class UnibeeApiMerchantSubscriptionOnetimeAddonPreviewReq
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiMerchantSubscriptionOnetimeAddonPreviewReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiMerchantSubscriptionOnetimeAddonPreviewReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [addon_id, quantity, apply_promo_credit, apply_promo_credit_amount, currency, discount_amount, discount_code, discount_percentage, gateway_id, gateway_payment_type, metadata, subscription_id, tax_percentage, user_id].hash
    end
