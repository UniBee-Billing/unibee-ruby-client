...
module Unibee
  class UnibeeApiMerchantPaymentNewReq
    # GatewayId
    ...
    def self.openapi_types
      {
        ...
        :'items' => :'Array<UnibeeApiMerchantPaymentItem>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiMerchantPaymentNewReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiMerchantPaymentNewReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'send_invoice')
        self.send_invoice = attributes[:'send_invoice']
      else
    ...
    def hash
      [gateway_id, cancel_url, country_code, currency, description, email, external_payment_id, external_user_id, gas_payer, gateway_payment_type, items, metadata, name, payment_type, payment_ui_mode, plan_id, quantity, redirect_url, send_invoice, total_amount, user_id].hash
    end
