...
module Unibee
  class UnibeeApiMerchantSubscriptionCreateReq
    # PlanId
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiMerchantSubscriptionCreateReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiMerchantSubscriptionCreateReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [plan_id, addon_params, apply_promo_credit, apply_promo_credit_amount, cancel_url, confirm_currency, confirm_total_amount, currency, discount, discount_code, email, external_user_id, free_in_initial_period, gateway_id, gateway_payment_type, metadata, payment_method_id, payment_ui_mode, product_data, quantity, return_url, split_payment, start_incomplete, tax_percentage, trial_end, user, user_id, vat_country_code, vat_number].hash
    end
