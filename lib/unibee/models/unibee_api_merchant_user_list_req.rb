...
module Unibee
  class UnibeeApiMerchantUserListReq
    # Count OF Page
    attr_accessor :count
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiMerchantUserListReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiMerchantUserListReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sub_status')
        if (value = attributes[:'sub_status']).is_a?(Array)
          self.sub_status = value
        end
      end
    ...
    def hash
      [count, create_time_end, create_time_start, delete_include, email, external_user_id, first_name, gateway_ids, last_name, page, plan_ids, search_key, sort_field, sort_type, status, sub_status, subscription_id, user_id].hash
    end
