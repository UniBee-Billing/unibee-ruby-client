...
module Unibee
  class UnibeeApiBeanDetailGatewayCompanyIssuer
    attr_accessor :issue_address

    attr_accessor :issue_company_name

    attr_accessor :issue_logo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiBeanDetailGatewayCompanyIssuer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiBeanDetailGatewayCompanyIssuer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'issue_logo')
        self.issue_logo = attributes[:'issue_logo']
      end
    ...
    def hash
      [issue_address, issue_company_name, issue_logo, issue_reg_number, issue_vat_number].hash
    end
