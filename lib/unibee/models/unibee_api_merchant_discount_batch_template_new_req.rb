...
module Unibee
  ...
  class UnibeeApiMerchantDiscountBatchTemplateNewReq
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiMerchantDiscountBatchTemplateNewReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiMerchantDiscountBatchTemplateNewReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [billing_type, code_prefix, discount_type, end_time, quantity, start_time, advance, currency, cycle_limit, discount_amount, discount_percentage, metadata, name, plan_apply_group, plan_apply_type, plan_ids, subscription_limit, upgrade_long_plan_only, upgrade_only, user_limit, user_scope].hash
    end
