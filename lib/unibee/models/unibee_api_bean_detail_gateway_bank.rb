...
module Unibee
  class UnibeeApiBeanDetailGatewayBank
    ...
    attr_accessor :aba_routing_number
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiBeanDetailGatewayBank` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiBeanDetailGatewayBank`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'aba_routing_number')
        self.aba_routing_number = attributes[:'aba_routing_number']
      end

      if attributes.key?(:'cnaps')
        self.cnaps = attributes[:'cnaps']
      end
    ...
    def hash
      [account_holder, address, aba_routing_number, cnaps, remarks, account_number, bank_name, bic, bsb_code, iban, swift_code].hash
    end
