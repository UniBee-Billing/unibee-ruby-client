...
module Unibee
  class UnibeeApiBeanMerchantBatchTask
    # create utc time
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiBeanMerchantBatchTask` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiBeanMerchantBatchTask`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [create_time, download_url, failure_reason, finish_time, format, id, last_update_time, member_id, merchant_id, payload, start_time, status, success_count, task_cost, task_name, task_type, upload_file_url].hash
    end
