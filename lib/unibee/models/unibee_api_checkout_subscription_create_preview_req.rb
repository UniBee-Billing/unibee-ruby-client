...
module Unibee
  class UnibeeApiCheckoutSubscriptionCreatePreviewReq
    # PlanId
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiCheckoutSubscriptionCreatePreviewReq` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiCheckoutSubscriptionCreatePreviewReq`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [plan_id, addon_params, apply_promo_credit, apply_promo_credit_amount, currency, discount_code, email, external_user_id, gateway_id, gateway_payment_type, quantity, tax_percentage, trial_end, user, user_id, vat_country_code, vat_number].hash
    end
