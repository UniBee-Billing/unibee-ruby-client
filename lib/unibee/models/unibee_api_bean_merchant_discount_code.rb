...
module Unibee
  class UnibeeApiBeanMerchantDiscountCode
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Unibee::UnibeeApiBeanMerchantDiscountCode` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Unibee::UnibeeApiBeanMerchantDiscountCode`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [advance, billing_type, code, create_time, currency, cycle_limit, discount_amount, discount_percentage, discount_type, end_time, id, is_deleted, merchant_id, metadata, name, plan_apply_group, plan_apply_type, plan_ids, quantity, start_time, status, upgrade_longer_only, upgrade_only, user_limit, user_scope].hash
    end
